...
from Shallow_Manipulation_Env import RobotEnv
...
class Worker(mp.Process):
  ...
  def run(self):
    ...
    self.env = RobotEnv(worker_id=self.wid,p_id=pybullet,dt=self.dt,maxSteps=20)
    ...
    while self.g_ep.value < MAX_EP:
      ...
      while True:
        ...
        observation_next, reward, done, suc = self.env.step(np.array([action[0],action[1],action[2]]))
        ...
        if total_step % (UPDATE_GLOBAL_ITER + self.wid) == 0 or done:# or self.g_ep.value % (10 * UPDATE_GLOBAL_ITER):
          #sync
        ...
        if total_step % 100 == 0:
          ...
          if self.wid == 0:
            print("total step %d, avg suc %f, avg reward %f" % (total_step, suc_check / 100.0, reward_check / 100.0))

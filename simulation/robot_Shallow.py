...
class Robot:
    def __init__(self,pybullet_api,start_pos=[0.4,0.3,0.4],urdf_path=None):
       ...
       model_path = os.path.join(urdf_path,"panda_robotiq_old.urdf")
    ...
    def getWrenchTipPos(self):
        ...
        wrench_tip_pos = 0.5 * np.array(left_tip_pos) + 0.5 * np.array(right_tip_pos)
        return wrench_tip_pos

    def getLeftGripperTipPos(self):
        left_tip_pos = self.p.getLinkState(self.robotId, self.gripper_left_tip_index)[0]
        return left_tip_pos
